...
let personSchema = new Schema({
  ...
  favoriteFoods: { type: [String], default: "Cheese" }
});
...
let createAndSavePerson = function(done) {
  let SidneyCrosby = new Person({
    name: "Sidney Crosby",
    age: 31,
    favoriteFoods: ["burgers", "fries", "Tim Hortons"]
  });
  
  SidneyCrosby.save((err, data) => {
    if(err) {
      return done(err);
    }
...
/**
 * So apparently we're not actually creating many people... >.>
Person.create(arrayOfPeople: [
  { name: "Matt Murray", age: 24, favoriteFoods: ["cheese", "fries", "candy bars"] },
  { name: "Evgeni Malkin", age: 32, favoriteFoods: ["pizza", "vodka food", "mum's home cooking"] },
  { name: "Phil Kessel", age: 31, favoriteFoods: ["hot dogs", "burgers", "pizza"] }
]);
*/
...
let queryChain = function(done) {
  ...
  .sort({ name: "asc" }) // Arrow functions have an implied return!
  .limit(2)
  .select("-age")
  .exec()
};
